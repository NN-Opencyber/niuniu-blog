...
**用法**: `/token-stats` 或说"查询 token 消耗"

**触发渠道**: 仅飞书和 OpenClaw Web 页面执行，其他渠道不显示不执行
...
**格式要点（禁止改动）**:
- 总计/今日: 亿万格式（如2亿1310.48万）
- 周期内: 显示"上次查询时间"+"XX分内消耗"+"发起调用XX次"
- 周期内最高/历史最高: Token、时间、模型分开显示
- 按模型: 按消耗降序排列

**脚本**: `~/.openclaw/scripts/token_stats.py`
**DB**: 192.168.35.10:5432/litellm
**记录文件**: /tmp/token_stats_last.json
